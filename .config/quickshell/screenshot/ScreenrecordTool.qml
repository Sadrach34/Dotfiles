...
PanelWindow {
    ...
    function getModes() {
        return [
            ...
            },
            {
                ...
                tooltip: ScreenRecorder.canRecordDirectly ? "Región" : "Región (No disponible sin config)",
                enabled: ScreenRecorder.canRecordDirectly
            },
            {
                name: "window",
                icon: Icons.windowScreenshot,
                tooltip: ScreenRecorder.canRecordDirectly ? "Ventana" : "Ventana (No disponible sin config)",
                enabled: ScreenRecorder.canRecordDirectly
            },
            {
                name: "screen",
                icon: Icons.fullScreenshot,
                tooltip: ScreenRecorder.canRecordDirectly ? "Pantalla completa" : "Pantalla (No disponible sin config)",
                enabled: ScreenRecorder.canRecordDirectly
            },
            {
                name: "portal",
    ...
    function open() {
        if (modeGrid)
            modeGrid.currentIndex = ScreenRecorder.canRecordDirectly ? 3 : 6;
    ...
    FocusScope {
        ...
        MouseArea {
            ...
            onPressed: mouse => {
                if (screenrecordPopup.currentMode === "screen" || screenrecordPopup.currentMode === "portal") return;
