...
spawn_terminal() {
  ...
  # Wait for window to appear (increased time for kitty)
  sleep 0.3
  ...
  # Fallback: buscar por clase de terminal en special:scratchpad
  # (NO usar focusHistoryID — apuntaría a la ventana más recientemente enfocada, ej. Firefox)
  if [ -z "$new_addr" ] || [ "$new_addr" = "null" ]; then
    TERM_CLASS=$(basename "$TERMINAL_CMD" | awk '{print $1}')
    new_addr=$(hyprctl clients -j | jq -r --arg cls "$TERM_CLASS" \
      '[.[] | select(.workspace.name == "special:scratchpad" and (.class | ascii_downcase | contains($cls | ascii_downcase)))] | sort_by(.pid) | last | .address // empty')
    debug_echo "Fallback by class '$TERM_CLASS' in scratchpad: $new_addr"
  fi
