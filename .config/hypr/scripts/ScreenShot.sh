...
shotswappy() {
	# Abre swappy directamente tras la selección (sin pasar por notificación)
	grim -g "$(slurp)" - | swappy -f -

	# Tras cerrar swappy: buscar el archivo más reciente guardado
	latest=$(ls -t "${dir}"/swappy-*.png 2>/dev/null | head -1)
	if [[ -n "$latest" ]]; then
		wl-copy <"$latest"
		"${sDIR}/Sounds.sh" --screenshot
		notify-send -t 4000 -i "${iDIR}/picture.png" \
			" Screenshot" " Guardado y copiado al portapapeles"
	fi
}
